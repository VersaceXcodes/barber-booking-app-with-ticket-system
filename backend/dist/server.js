...
app.get('/api/availability', async (req, res) => {
    try {
        ...
        const bookingsResult = await pool.query('SELECT appointment_date, COUNT(*) as booked_count FROM bookings WHERE appointment_date >= $1 AND appointment_date <= $2 AND status = $3 GROUP BY appointment_date', [start_date, end_date, 'confirmed']);
...
app.get('/api/bookings/search', async (req, res) => {
    try {
        ...
        if (phone && date) {
            const result = await pool.query('SELECT * FROM bookings WHERE customer_phone = $1 AND appointment_date = $2 ORDER BY appointment_time ASC', [phone, date]);
            return res.json({ bookings: result.rows, total: result.rows.length });
        }
...
app.get('/api/bookings/:ticket_number', async (req, res) => {
    try {
        ...
        const result = await pool.query(`SELECT b.*, s.name as service_name, s.price as service_price
       FROM bookings b
