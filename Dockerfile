...
RUN sed -i 's|VITE_API_BASE_URL=.*|VITE_API_BASE_URL=https://barber-booking-app-with-ticket-system.launchpulse.ai|g' .env 2>/dev/null || true
RUN sed -i 's|VITE_BACKEND_URL=.*|VITE_BACKEND_URL=https://barber-booking-app-with-ticket-system.launchpulse.ai|g' .env 2>/dev/null || true
RUN echo 'VITE_API_BASE_URL=https://barber-booking-app-with-ticket-system.launchpulse.ai' > .env.production.local && echo 'VITE_BACKEND_URL=https://barber-booking-app-with-ticket-system.launchpulse.ai' >> .env.production.local
RUN npm run build
