...
export const createTestService = async (token, overrides = {}) => {
...
describe('POST /api/auth/register', () => {
    ...
    it('should generate verification token', async () => {
        ...
        const result = await pool.query('SELECT verification_token, verification_token_expiry FROM users WHERE email = $1', ['verify@example.com']);
...
describe('POST /api/auth/login', () => {
    ...
    it('should compare passwords as plain text (no bcrypt)', async () => {
        ...
        await pool.query(`INSERT INTO users (user_id, email, password_hash, name, phone, is_verified, created_at, updated_at)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $7)`, ['test-user-1', 'plaintext@test.com', 'mypassword', 'Test', '+15551234567', true, new Date().toISOString()]);
        const response = await request(app)
            .post('/api/auth/login')
            .send({
...
describe('GET /api/availability/:date', () => {
    ...
    it('should reflect capacity overrides', async () => {
        // Create override
        await pool.query(`INSERT INTO capacity_overrides (override_id, override_date, time_slot, capacity, is_active, created_at, updated_at)
       VALUES ($1, $2, $3, $4, $5, $6, $6)`, ['test-override', '2024-06-20', '10:00', 1, true, new Date().toISOString()]);
...
describe('Database Transactions', () => {
    it('should rollback on error', async () => {
        ...
        try {
            ...
            await client.query('INSERT INTO users (user_id, email, password_hash, name, phone, is_verified, created_at, updated_at) VALUES ($1, $2, $3, $4, $5, $6, $7, $7)', ['test-tx-1', 'tx@test.com', 'pass', 'Test', '+15551234567', false, new Date().toISOString()]);
            // Force error with duplicate email
            await client.query('INSERT INTO users (user_id, email, password_hash, name, phone, is_verified, created_at, updated_at) VALUES ($1, $2, $3, $4, $5, $6, $7, $7)', ['test-tx-2', 'tx@test.com', 'pass', 'Test', '+15551234567', false, new Date().toISOString()]);
...
describe('Admin Dashboard', () => {
    ...
    beforeAll(async () => {
        // Create admin user
        await pool.query(`INSERT INTO users (user_id, email, password_hash, name, phone, is_verified, created_at, updated_at)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $7)`, ['admin-1', 'admin@barberslot.com', 'admin123', 'Admin', '+15551234567', true, new Date().toISOString()]);
...
describe('Performance Tests', () => {
    ...
    it('should handle 10 concurrent booking requests', async () => {
        const promises = Array.from({ length: 10 }, (_, i) => request(app)
            .post('/api/bookings')
            .send({
