...
app.get('/api/availability/:date', async (req, res) => {
  try {
    ...
    const bookingsResult = await pool.query(
      'SELECT appointment_time, COUNT(*) as booked_count FROM bookings WHERE appointment_date = $1 AND status = $2 GROUP BY appointment_time',
      [date, 'confirmed']
    );
    const bookingCounts = {};
    bookingsResult.rows.forEach(row => {
...
app.get('/api/bookings/search', async (req, res) => {
  try {
    ...
    return res.status(400).json(createErrorResponse(
      "Provide either 'ticket_number' OR both 'phone' and 'date'",
      null,
  ...
  try {
    ...
    const updateResult = await pool.query(
      `UPDATE bookings
       ...
       WHERE booking_id = $4
       RETURNING *`,
      [now, cancelled_by, cancellation_reason, booking.booking_id]
    );
...
app.post('/api/bookings/:ticket_number/reschedule', async (req, res) => {
  try {
    ...
    const availabilityRes = await pool.query(
      'SELECT COUNT(*) as count FROM bookings WHERE appointment_date = $1 AND appointment_time = $2 AND status = $3',
      [new_appointment_date, new_appointment_time, 'confirmed']
    );
    const currentBookings = parseInt(availabilityRes.rows[0].count);
    ...
    await pool.query(
      `UPDATE bookings
       ...
       WHERE booking_id = $3`,
      [now, `Rescheduled to ${newTicketNumber}`, original.booking_id]
    );
...
app.post('/api/auth/register', async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `INSERT INTO users (user_id, email, password_hash, name, phone, is_verified, verification_token, verification_token_expiry, created_at, updated_at)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9, $9)
       RETURNING user_id, email, name, phone, is_verified, created_at`,
      [user_id, email.toLowerCase(), password, name, phone, false, verification_token, verification_token_expiry.toISOString(), now]
    );
...
app.post('/api/auth/verify-email', async (req, res) => {
  try {
    ...
    const now = new Date().toISOString();
    const updateResult = await pool.query(
      `UPDATE users SET is_verified = TRUE, verification_token = NULL, verification_token_expiry = NULL, updated_at = $1
       WHERE user_id = $2
       RETURNING user_id, email, name, phone, is_verified, created_at, updated_at`,
      [now, user.user_id]
    );
...
app.patch('/api/auth/me', authenticateToken, async (req: any, res) => {
  try {
    ...
    const result = await pool.query(
      `UPDATE users SET ${updates.join(', ')} WHERE user_id = $${paramCount} RETURNING user_id, email, name, phone, is_verified, created_at, updated_at`,
      params
    );
...
app.patch('/api/admin/services/:service_id', authenticateAdmin, async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `UPDATE services SET ${updates.join(', ')} WHERE service_id = $${paramCount} RETURNING *`,
      params
    );
  ...
  try {
    ...
    const result = await pool.query(
      `INSERT INTO capacity_overrides (override_id, override_date, time_slot, capacity, is_active, created_at, updated_at)
       ...
       RETURNING *`,
      [override_id, override_date, time_slot, capacity, is_active, now]
    );
...
app.patch('/api/admin/capacity-overrides/:override_id', authenticateAdmin, async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `UPDATE capacity_overrides SET ${updates.join(', ')} WHERE override_id = $${paramCount} RETURNING *`,
      params
    );
...
app.get('/api/admin/dashboard/stats', authenticateAdmin, async (req, res) => {
  try {
    ...
    const todayBookings = await pool.query(
      "SELECT COUNT(*) as count, status FROM bookings WHERE appointment_date = $1 GROUP BY status",
      [todayStr]
    );
    ...
    todayBookings.rows.forEach(row => {
      ...
      if (row.status === 'completed') stats.today_completed = count;
      if (row.status === 'confirmed') stats.today_upcoming = count;
    });
...
app.get('/api/admin/reports/bookings', authenticateAdmin, async (req, res) => {
  try {
    ...
    const byService: { [key: string]: { service_name: string; count: number } } = {};
    bookings.forEach(b => {
    ...
    const byDayOfWeek: { [key: string]: { day: string; count: number } } = {
    ...
    const byTimeSlot: { [key: string]: { time: string; count: number; total_capacity: number } } = {};
    bookings.forEach(b => {
...
app.post('/api/queue/leave/:queue_id', async (req, res) => {
  try {
    ...
    if (queueEntry.status !== 'waiting') {
      return res.status(400).json(createErrorResponse(
        'Can only leave queue when in waiting status',
        null,
        'INVALID_STATUS'
      ));
    }
